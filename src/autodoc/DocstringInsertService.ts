...
export default class DocstringInsertService {
  ...
  public async updateDocstrings(document: vscode.TextDocument) {
    ...
    try {
      ...
      let taggedFunctions: { function: Function; tag: DocTag }[] =
        this.getFunctionTags(functionsToDocument);
      ...
      if (autoFunctions.length > 0) {
        ...
        let reparseFunctions: Function[] =
          await this.codeParserService.parseNoTrack(document);

        docstrings
          .filter((docPair) => {
            return reparseFunctions.find(
              (func) => hashFunction(func) == hashFunction(docPair.function)
            );
          })
          .map((docPair) => {
            try {
              docPair.function = reparseFunctions.find(
                (func) => hashFunction(func) == hashFunction(docPair.function)
              )!;
            } finally {
            }
