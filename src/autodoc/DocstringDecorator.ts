...
export default class DocstringDecorator implements vscode.Disposable {
  ...
  private registerDocrationTypes() {
    ...
    this.decorations["function.body"] =
  ...
  public applyDocstringRecommendations(
    functions: Function[],
    ...
    try {
      ...
      let pushDecorationByRange = (key: string, range: vscode.Range) => {
        let currDecoration = (matchDecorations[key] =
          matchDecorations[key] || []);
      ...
      functions.forEach((func) => {
        let recommendedHeaderPosition = new vscode.Position(
          func.definition_line,
          0
        );
        let recommendedDocstringPos = doc.offsetAt(
          recommendedHeaderPosition.translate(1, 0)
        );
